...
public class LogoutTests
{
    ...
    [TestMethod]
    public async Task LogoutLogsUserOut()
    {
        ...
        Assert.IsTrue(loginResult.ResponseBody.UserLoggedIn);

        var logoutResult = await client.Logout(new() { Username = "kermit" });
        Assert.IsFalse(logoutResult.HasError);
